...
export const analyzeResume = asyncHandler(async (req, res) => {
  // 1. Destructure jobTitle from req.body
  ...
  // Validation: jobTitle ko bhi required check mein add kiya hai
  if (!resumeId || !jobDescription || !jobTitle) {
    throw new ApiError(400, "Resume ID, Job Description, and Job Title are required");
  }
  ...
  const candidateSkills = aiResult.parsedData.skills || [];
  const requiredSkills = aiResult.missingSkills.concat(candidateSkills); 
  ...
  // --- SAVING DATA ---
  resume.analysis = {
    atsScore: finalScore, // Final Hybrid score save ho raha hai
    summary: aiResult.summary,
  ...
  // Yahan jobTitle update ho raha hai
...
export const getAllUserAnalyses = asyncHandler(async (req, res) => {
  const userId = req.user._id;

  // Saare resumes fetch karein jo is user ke hain
  // .sort({ createdAt: -1 }) se latest analysis sab se upar aayega
  const analyses = await Resume.find({ 
    owner: userId,
    status: "completed" // Sirf wahi dikhayein jo analyze ho chukay hain
  }).sort({ createdAt: -1 });
  ...
  return res
    .status(200)
    .json(new ApiResponse(200, analyses, "All analysis results fetched successfully"));
});

// get all resume analisys result 